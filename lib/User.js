...
class User extends BaseModel {
	...
	static get (user, appId, callback) {
		if (user.id) {
			Services.datasource.dataStorage.getObjects([user.id], (errs, results) => {
			...
			Services.datasource.dataStorage.searchObjects({modelName: 'user', filters}, (err, results) => {
				if (err) {
					return callback(err);
				}

				if (!results.length) {
					return callback(new NexxusError(NexxusError.errors.UserNotFound));
				}
			...
			if (err && err.status === 404) {
				Services.datasource.dataStorage.createObjects([newUser.properties, userMetadata], errs => {
					if (errs && errs.length) {
						...
						return callback(new NexxusError(NexxusError.errors.ServerFailure, ['failed to create user.']));
					}

					return callback(null, newUser);
				});
			} else {
				callback(new NexxusError(NexxusError.errors.UserAlreadyExists));
			}
		});
	}
	...
	static update (patches, callback) {
		Services.datasource.dataStorage.updateObjects(patches, (errs, dbObjects) => {
			...
			return callback(null, dbObjects[objId]);
		});
	}
