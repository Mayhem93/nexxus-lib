...
ElasticSearchDB.prototype.getQueryObject = function(builder) {
	if (!(builder instanceof FilterBuilder)) {
		throw new TelepatError(TelepatError.errors.ServerFailure,
			['getQueryObject: 1st argument is not instanceof FilterBuilder']);
	}
			...
			if (child instanceof BuilderNode) {
				...
				if (translationMappings[replaced]) {
					...
					if (replaced == 'not') {
						...
						child[translationMappings[replaced]] = {field: child[replaced]};
					} else if (replaced === 'in_array') {
						var filteredField = Object.keys(child[replaced])[0];

						if (Array.isArray(child[replaced][filteredField])) {
							child[translationMappings[replaced]] = child[replaced];
						} else {
							var subQueryObject = {};
							subQueryObject[filteredField] = [child[replaced][filteredField]];

							child[translationMappings[replaced]] = subQueryObject;
						}
