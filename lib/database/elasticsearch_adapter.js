const MainDatabaseAdapter = require('./mainDatabase');
...
class ElasticSearchDB extends MainDatabaseAdapter {
	constructor (config) {
		...
		this.connection.ping({
			...
			if (err) {
				Application.logger.error(`Failed connecting to Elasticsearch "${this.config.host}": ${err.message}. Retrying...`);
	...
	createObjects (objects, callback) {
		const bulk = [];
		const builtinModels = ['application', 'admin', 'user', 'user_metadata', 'context'];
	...
	updateObjects (patches, callback) {
		...
		function getAndUpdate (objectIds, callback2) {
			...
			async.series([
				callback1 => {
					this.getObjects(Object.keys(objectIds), (err, results, versions) => {
						if (err && err.length === 1) {
