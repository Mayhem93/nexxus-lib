...
class ElasticSearchDB extends MainDatabaseAdapter {
	...
	async getObjects (ids) {
		if (!Array.isArray(ids) || ids.length === 0) {
			throw new TelepatError(TelepatError.errors.InvalidFieldValue, 'ElasticSearchDB.getObjects: "ids" should be a non-empty array');
		}
		...
		results.docs.forEach(result => {
			if (result.found) {
				...
				errors.push(new TelepatError(TelepatError.errors.ObjectNotFound, [result._id]));
			}
		});
	...
	async createObjects (objects) {
		if (!Array.isArray(objects) || objects.length === 0) {
			throw new TelepatError('InvalidFieldValue', ['ElasticSearchDB.createObjects: "objects" should be a non-empty array']);
		}
		...
		objects.forEach(obj => {
			...
			if (obj.type && obj.id) {
				...
				errors.push(new TelepatError('MissingRequiredField', ['"id" or "type"']));
			}
		});
		...
		if (res.errors) {
			res.items.forEach(error => {
				errors.push(new TelepatError('ServerFailure', `Error creating ${error.index._type}: ${error.index.error}`));
			});
		}
	...
	async updateObjects (patches) {
		if (!Array.isArray(patches) || patches.length === 0) {
			throw new TelepatError(TelepatError.errors.InvalidFieldValue, 'ElasticSearchDB.updateObjects: "patches" should be a non-empty array');
		}
		...
		patches.forEach(patch => {
			if (!patch.path || typeof patch.path !== 'string') {
				return errors.push(new TelepatError(TelepatError.errors.InvalidPatch, ['path is missing or invalid']));
			} else if (patch.path.split('/').length !== 3) {
				return errors.push(new TelepatError(TelepatError.errors.InvalidPatch, ['the path is malformed']));
			}
	...
	async deleteObjects (ids) {
		if (typeof ids !== 'object') {
			throw new TelepatError(TelepatError.errors.InvalidFieldValue, 'deleteObjects must be supplied an object');
