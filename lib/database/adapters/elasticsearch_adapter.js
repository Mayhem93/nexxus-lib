...
class ElasticSearchDB extends MainDatabase {
	...
	[processSchemaModificationMethod] (modifications) {
		/* const res = await this.connection.indices.create({
			index: `${constants.CHANNEL_KEY_PREFIX}-${objects[0].id}-${modelName}`
		});
	...
	async updateObjects (patches) {
		...
		async function getAndUpdate (objectPatches) {
			...
			results.forEach(dbObject => {
				const diff = NexxusPatch.applyPatches(objectsToGet.get(dbObject.properties.id).patches, dbObject);
				let index;

				if (modifiedApplicationSchemas.has(dbObject.properties.id)) {
					try {
						// we don't want this to be synchronous
						this[processSchemaModificationMethod](diff);
					} catch (err) {
						Services.logger.warning(`Unable to process schema modifications: ${err}`);
					}
				}

				finalResults.set(dbObject.properties.id, dbObject);

				switch (dbObject.properties.type) {
					case 'application':
					case 'admin': {
						index = `${constants.CHANNEL_KEY_PREFIX}-${dbObject.properties.type}`;
						shouldRefresh = true;

						break;
					}
					default: {
						index = `${constants.CHANNEL_KEY_PREFIX}-${dbObject.properties.application_id}-${dbObject.properties.type}`;
					}
				}

				bulk.push({ update: { _id: dbObject.properties.id, _version: versions[dbObject.properties.id], _index: index, _type: '_doc' } });
				bulk.push({ doc: diff });
			});
