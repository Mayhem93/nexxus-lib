...
class ElasticSearchDB extends MainDatabase {
	...
	async searchObjects (modelName, applicationId, options) {
		const reqBody = {
			query: {
				filtered: {
					filter: {}
	...
	async createObjects (objects) {
		...
		objects.forEach(obj => {
			...
			if (obj.application_id && obj.id) {
				...
				bulk.push({ index: { _id: obj.id, _index: `${constants.CHANNEL_KEY_PREFIX}-${obj.application_id}-${modelName}` } });
				bulk.push(obj);
			} else {
				errors.push(new NexxusError('MissingRequiredField', ['"id" or "application_id"']));
			}
		});
	...
	async updateObjects (applicationId, patches) {
		...
		async function getAndUpdate (objectPatches) {
			...
			results.forEach(dbObject => {
				...
				bulk.push({ update: { _id: dbObject.id, _version: versions[dbObject.id], _index: `${constants.CHANNEL_KEY_PREFIX}-${dbObject.application_id}-${dbObject.type}` } });
