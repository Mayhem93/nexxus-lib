...
class ElasticSearchDB extends MainDatabase {
	...
	async getObjects (items) {
		if (!(items instanceof Map) || items.length === 0) {
			throw new NexxusError(NexxusError.errors.InvalidFieldValue, 'ElasticSearchDB.getObjects: "ids" should be a non-empty Map');
		}
