...
const {BuilderNode} = require('../../../index').FilterBuilder;
const {Delta, TelepatError, Model, logger} = require('../../../index');
...
class ElasticSearchDB extends MainDatabaseAdapter {
	constructor (config) {
		...
		this.connection.ping({
			...
			if (err) {
				logger.error(`Failed connecting to Elasticsearch "${this.config.host}": ${err.message}. Retrying...`);
				setTimeout(retryConnection, 1000);
			} else {
				logger.info('Connected to ElasticSearch MainDatabase');
				this.emit('ready');
			}
		});
	}
		...
		return this.connection.bulk({
			...
			if (res.errors) {
				res.items.forEach(error => {
					logger.error(`Error creating ${error.index._type}: ${error.index.error}`);
				});
			}
