	...
	constructor (config) {
		...
		const tryConnect = callback => {
			amqplib.connect(`amqp://${this.config.user}:${this.config.password}@${this.config.host}`, (err, conn) => {
				if (err) {
					logger.error(`Failed connecting to AMQP messaging queue (${err.toString()}). Retrying... `);
		...
		const tryChannel = callback => {
			this.connectionClient.createChannel((err, ch) => {
				if (err) {
					logger.error(`Failed creating channel on the AMQP messaging queue (${err.toString()}). Retrying... `);
		...
		async.series([
			...
			callback => {
				this.amqpChannel.bindQueue(this.broadcastQueue || this.config.serviceType, 'amq.fanout', '', {}, err => {
					if (err) {
						logger.error('Failed to bind AMQP queue to amq.fanout');
						callback(err);
					} else {
						logger.info('Connected to AMQP Messaging Queue');
						callback();
					}
				});
			}
