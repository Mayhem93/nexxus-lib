...
class AMQPClient extends MessagingClient {
	constructor (config) {
		...
		// we need to disable this eslint rule, otherwise async lib doesn't wait for a promise and works with callbacks instead
		const tryConnect = async () => { // eslint-disable-line require-await
			return new Promise(async resolve => {
				try {
					...
					Services.logger.emergency(`Failed connecting to AMQP messaging queue (${err.toString()}). Retrying... `);

					return setTimeout(async () => { await tryConnect(); }, 1000);
				}
			});
		};
