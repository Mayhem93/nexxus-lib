...
class Model extends BaseModel {
	constructor (props) {
		...
		const proxiedParent = super(props, ['context_id', 'application_id', 'user_id']);
	...
	static async create (objects) {
		...
		async.series([
			callback1 => {
				async.forEachOf(objects, (d, i, c) => {
					objects[i].object.id = guid.v4();
					let object = new Model(objects[i].object);

					if (object instanceof NexxusError) {
						return c();
					}
				...
				if (!Object.keys(objParentInfo).length) {
					...
					Services.datasource.dataStorage.getObjects(Object.keys(objParentInfo), (errs, results) => {
						if (errs && errs.length >= 1) {
							errs.forEach(error => {
								if (error && error.status === 404) {
									Services.logger.warning((new NexxusError(NexxusError.errors.ParentObjectNotFound, [error.args[0], error.args[1]])).message);

									return delete objParentInfo[error.args[1]];
								}
						...
						if (results && results.length >= 1) {
							...
							objects.forEach(delta => {
								let obj = delta.object;

								if (Application.apps[obj.application_id].modelSchema(obj.type).isValidModel() instanceof NexxusError) {
									return;
								}
								...
								if (obj[Model.hasSome(obj)] && obj[Model.hasSome(obj)].length <= objParentInfo[obj.id].relationKeyLength) {
									Services.logger.warning((new NexxusError(NexxusError.errors.InvalidObjectRelationKey,
										[
	...
	static modelCountByChannel (channel, aggregation, callback) {
		...
		if (aggregation) {
			cachingKey += (Buffer.from(JSON.stringify(aggregation))).toString('base64');
		}
		...
		let waterfall = waterfallCallback => {
			async.waterfall([
				callback1 => {
					Services.redisClient.get(cachingKey, callback1);
				},
				(result, callback1) => {
					if (result) {
						...
						async.waterfall([
							callback2 => {
								Services.redisClient.set([`${cachingKey}:LOCK`, '1', 'NX'], callback2);
							},
							...
							(count, callback2) => {
								let tranzaction = Services.redisClient.multi();
								...
								tranzaction.exec(err2 => {
									if (err2) {
										Services.redisClient.del([`${cachingKey}:LOCK`], () => {
