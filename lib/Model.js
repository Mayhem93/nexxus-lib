...
class Model extends BaseModel {
	...
	static async get (objects) {
		const {errors, results} = await Services.datasource.dataStorage.getObjects([objects]);
	...
	static update (patches) {
		return Services.datasource.dataStorage.updateObjects(patches);
	}

	static async delete (objects) {
		const { errors } = await Services.datasource.dataStorage.deleteObjects(objects);

		return errors;
	}

	static count (applicationId, modelName) {
		let filter = new FilterBuilder();

		filter.addFilter('is', { application_id: applicationId });
	...
	static countByChannel (channel, aggregation) {
	...
	static async search (channel, sort, offset, limit) {
		let searchFilters = Model.getFilterFromChannel(channel);

		return (await Services.datasource.dataStorage.searchObjects({
			filters: searchFilters,
			modelName: channel.props.model,
			sort,
			offset,
			limit
		})).results.map(object => new Model(object));
	}
