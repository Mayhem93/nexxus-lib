...
class Channel {
	...
	model (model, id) {
		if (model) {
			if (Application(this.applicationId).hasModel(model)) {
	...
	isValid () {
		...
		if (this.mask === 4 && constants.builtinModels.indexOf(this.props.model) === -1)	{
	...
	get (options) {
		...
		validChannel = validChannel.replace('{appId}', this.applicationId);
		...
		if (this.filter) {
			validChannel += `:filter:${(Buffer.from(JSON.stringify(this.filter))).toString('base64')}`;
		}
	...
	clone () {
		let c = new Channel(this.applicationId, this.props);
...
Channel.validChannels = {
	4: `${constants.CHANNEL_KEY_PREFIX}:{appId}:{model}`,												// channel used for built-in models (users, contexts)
	5: `${constants.CHANNEL_KEY_PREFIX}:{appId}:context:{context}:{model}`,							// the Channel of all objects from a context
	7: `${constants.CHANNEL_KEY_PREFIX}:{appId}:context:{context}:users:{user_id}:{model}`,			// the Channel of all objects from a context from an user
	12: `${constants.CHANNEL_KEY_PREFIX}:{appId}:{parent_model}:{parent_id}:{model}`,					// the Channel of all objects belong to a parent
	14: `${constants.CHANNEL_KEY_PREFIX}:{appId}:users:{user_id}:{parent_model}:{parent_id}:{model}`,	// the Channel of all comments from event 1 from user 16
	20: `${constants.CHANNEL_KEY_PREFIX}:{appId}:{model}:{id}`											// the Channel of one item
};
