...
let NexxusError = require('./NexxusError');
...
class Subscription {
			...
			callback1 => {
				...
				if (device.volatile && device.volatile.active) {
					...
					if (!transportType || !token) {
						return callback1(new NexxusError(NexxusError.errors.DeviceInvalid, [deviceId, 'volatile server_name or token is missing']));
					}
				} else {
					if (!device.persistent || !device.persistent.type || !device.persistent.token) {
						return callback1(new NexxusError(NexxusError.errors.DeviceInvalid, [deviceId, 'persistent type and/or token is missing']));
					}
				...
				return Services.redisClient.sadd([channel.get(), `${transportType}|${deviceId}|${token}|${appId}`], callback1);
			},
			callback1 => {
	...
	static remove (appId, deviceId, channel, token, callback) {
		...
		async.series([
			...
			callback1 => {
				if (removed === 0) {
					return callback1(new NexxusError(NexxusError.errors.SubscriptionNotFound));
				}
	...
	static removeAllSubscriptionsFromDevice (appId, deviceId, token, transport, callback) {
		...
		if (!transport || typeof transport !== 'string') {
			return callback(new NexxusError(NexxusError.errors.UnspecifiedError, ['removeAllSubscriptionsFromDevice: need to specify transport']));
		}
		...
		return async.series([
			...
			callback1 => {
				...
				return transaction.exec(callback1);
			}
		], err => {
	...
	static getDevice (appId, id, callback) {
		...
		Services.redisClient.get(key, (err, result) => {
			...
			if (result === null) {
				return callback(new NexxusError(NexxusError.errors.DeviceNotFound, [id]));
			}
	...
	static updateDevice (appId, device, props, callback) {
		...
		Subscription.getDevice(appId, device, (err, dev) => {
			...
			return Services.redisClient.set([key, JSON.stringify(newDevice), 'XX'], callback);
		});
	}
