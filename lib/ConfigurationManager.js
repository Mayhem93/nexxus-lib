...
class ConfigurationManager {
	...
	async load () {
		try {
			...
			throw new NexxusError(NexxusError.errors.ServerConfigurationFailure, [e.message]);
		}
	...
	*/
	[validate] (spec, config, rootName) {
		...
		for (const s in spec) {
			...
			if (result instanceof NexxusError) {
				throw result;
			}
		}
	...
	*/
	[verifyVariable] (variable, specVariable, rootName) {
		...
		if (specVariable.optional) {
			...
			return new NexxusError(NexxusError.errors.ServerConfigurationFailure, [`${fullVarName} is mising from the configuration`]);
		} else if (!variable && specVariable.exclusive_group) {
			return true;
		}

		if (specVariable.enum && !this[enumVerifier](variable, specVariable.enum)) {
			return new NexxusError(NexxusError.errors.ServerConfigurationFailure, [
