...
class NexxusApplication extends BaseModel {
	...
	constructor (props) {
		...
		this.contexts = {
			new: Context.new(this.properties.id),
		...
		this.users = {
			new: (prop, callback) => {
				prop.application_id = this.properties.id;
				Users.new(prop, callback);
			},
			get: (user, callback) => {
				Users.get(user, this.properties.id, callback);
			},
			update: Users.update,
			delete: Users.delete
		};
	...
	static async new (props) {
		...
		NexxusApplication.apps[props.id] = app;
	...
	async delete () {
		...
		delete NexxusApplication.apps[this.properties.id];
	...
	async update (patches) {
		...
		NexxusApplication.apps[this.properties.id] = application;
		await promisify(Services.redisClient.del)(`blg:application:${this.properties.id}`);
	}
	...
	static async getAll () {
		...
		(await Services.datasource.dataStorage.searchObjects({ modelName: 'application', offset, limit })).forEach(app => {
			NexxusApplication.apps[app.id] = new NexxusApplication(app);
		});

		return NexxusApplication.apps;
	}
	...
	static isAdmin (admin) {
		for (let r in NexxusApplication.apps) {
			if (NexxusApplication.apps[r].admins.indexOf(admin.id) !== -1) {
				return true;
			}
		}
	...
	async deleteModel (modelName) {
		if (!NexxusApplication.apps[this.properties.id].schema[modelName]) {
			throw new NexxusError('ApplicationSchemaModelNotFound', [this.properties.id, modelName]);
		}

		delete NexxusApplication.apps[this.properties.id].schema[modelName];
...
NexxusApplication.contexts = require('./Context');
