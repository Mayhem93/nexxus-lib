...
class NexxusApplication extends BaseModel {
	...
	async updateSchema (schemaObject) {
		if (!(schemaObject instanceof NexxusApplicationSchema)) {
			schemaObject = new NexxusApplicationSchema(schemaObject);
		}

		const patch = new NexxusPatch({
			op: 'replace',
			path: `application/${this.properties.id}/schema`,
			value: schemaObject.schema
		});

		let {errors} = await Services.datasource.dataStorage.updateObjects([patch]);

		if (errors[0]) {
			throw errors[0];
		}

		await promisify(Services.datasource.cacheStorage.del)(`blg:application:${this.properties.id}`);
	}
