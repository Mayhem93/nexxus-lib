...
/**
 * ACL enforcement used by the model/subscription route handlers. It runs AFTER
 * request validation (unlike a pre-route middleware) so it can see the resolved
 * model, target id, and incoming body — everything a row/action decision needs.
 *
 ...
 */
export class NexxusApiAcl {
  private static readonly LOG_LABEL = 'NxxApiAcl';
   ...
   * `NexxusApiModelParams.toDatabaseFilter`, or hand it to `enforceRowConstraint`
   * for a single object (`get`/`create`/`update`/`delete`).
   *
  ...
  public static authorize(
    ...
    if (!result.allowed) {
      NexxusApiAcl.logAndDeny(req, action, model, 'no granted role allows this action on the model');
    }
   ...
   */
  public static enforceRowConstraint(
    app: NexxusApplication,
    req: NexxusApiRequest,
    action: NexxusAclAction,
    model: string,
    ...
    if (data === null) {
      NexxusApiAcl.logAndDeny(req, action, model, 'target object not found');
    }

    const matches = new NexxusFilterQuery(constraint, app.getAppModelSchema(model)).test(data);

    if (!matches) {
      NexxusApiAcl.logAndDeny(req, action, model, 'object does not satisfy the row condition');
    }
  }
   ...
   */
  public static async loadObjectAttributes(
    appId: string,
    model: string,
    id: string,
   ...
   */
  public static subscriptionFilter(
    app: NexxusApplication,
    model: string,
  ...
  private static logAndDeny(req: NexxusApiRequest, action: NexxusAclAction, model: string, reason: string): never {
    NexxusApi.logger.info(
      `ACL deny: ${action} "${model}" — user "${req.user?.id ?? 'anonymous'}" ` +
      `(userType "${req.user?.userType ?? 'default'}"): ${reason}`,
      NexxusApiAcl.LOG_LABEL,
    );
