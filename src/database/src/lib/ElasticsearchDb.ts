...
export class NexxusElasticsearchDb extends NexxusDatabaseAdapter<ElasticsearchConfig, ElasticSearchEvents, NexxusElasticsearchDbStats> {
  ...
  public searchItems(options: NexxusDbSearchOptions<'acl'>): Promise<NexxusAclRole[]>;
    ...
    return this.esRequestWrapper(async () => {
      ...
      const models: Array<AnyNexxusModel> = searchResults.hits.hits.map(res => {
        switch (options.type) {
          ...
          case 'acl':
            return new NexxusAclRole(res._source as INexxusAclRole);
  ...
  public getItems(options: NexxusDbGetOptions<'acl'>): Promise<Array<NexxusAclRole | null>>;
      ...
      try {
        ...
        return esMgetResponse.docs.map(doc => {
          ...
          switch(options.type) {
            ...
            case 'acl':
              return new NexxusAclRole(doc._source as INexxusAclRole);
